...
module.exports = async (school) => {
  ...
  const result = await fetch('https://hcs.eduro.go.kr/school?' + queryParams.toString(), '')
    .then(res => res.json())
    .then(json => {
      ...
      else { return { schoolCode: json.schulList[0].orgCode, requestUrl: json.schulList[0].atptOfcdcConctUrl, schoolName: json.schulList[0].kraOrgNm, schoolAddr: json.schulList[0].addres } }
    })
